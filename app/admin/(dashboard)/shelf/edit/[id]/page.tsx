...
import { Plus, Trash2, ChevronDown, ChevronUp, BookOpen, FileText, ScrollText, Loader2, ArrowLeft } from 'lucide-react'
import Link from 'next/link'

export default function EditShelfItemPage() {
    ...
    <div className="max-w-4xl pb-32">
      <div className="mb-8 flex justify-between items-end">
        <div>
          ...
          <h1 className="text-3xl font-serif text-slate-900">Refine Collection</h1>
          <p className="text-slate-500 mt-2">Editing entry: <span className="text-slate-900 font-bold">{formData.title}</span></p>
        </div>
      </div>
          ...
          <div className="pt-4">
            {formData.reader_mode === 'pdf' && (
              <div className="space-y-4 p-6 bg-slate-50 rounded-xl border border-slate-100">
                <Label className="text-slate-900 font-bold">Update Manuscript (PDF)</Label>
                <FileUploader 
                  bucket="media"
                  folder="shelf_pdfs"
